  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-zinc-950/80 p-6 backdrop-blur-sm">
      <div className="w-full max-w-md space-y-4 rounded-xl border border-zinc-700 bg-zinc-900 p-5 shadow-xl">
        <div>
          <p className="font-medium text-primary">{t("trayDiscovery.setupComplete")}</p>
          <p className="mt-2 text-sm text-zinc-300">{t("trayDiscovery.runningInTray")}</p>
        </div>
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          >
            {hiding ? t("trayDiscovery.hiding") : t("trayDiscovery.hideToTray")}
          </button>
        </div>
      </div>
    </div>
